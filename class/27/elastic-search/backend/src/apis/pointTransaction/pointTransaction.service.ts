...
export class PointTransactionService {
  ...
  async create({ impUid, amount, currentUser: user }) {
    ...
    await queryRunner.startTransaction("SERIALIZABLE"); //4단계 적용: 동시에 접근으로 인한 데이터 오염 해결
    ...
    try {
      ...
      const userFound = await queryRunner.manager.findOne(
        User, {id: user.id}, {lock: {mode: "pessimistic_write"}}
      )
      const updatedUser = await this.userRepository.create({
        ...userFound, point: userFound.point + amount
      })
