...
export class ProductResolver {
  ...
  @Query(() => String)
  async fetchProducts(
    @Args({ name: "search", nullable: true }) search: string //검색어 입력이 필수는 아님
  ) {
    const result: any = await this.elasticsearchService.search({
      index: "myproduct033", //어떤 collection
      query: { //어떤 조건들
        bool: {
    ...
    console.log(JSON.stringify(result.hits.hits[0]._source.name, null, " "))
