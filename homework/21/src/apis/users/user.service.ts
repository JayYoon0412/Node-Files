...
export class UserService {
  ...
  async create({ userInput }) {
    const { areasId, temperatureId, userNumber, password, ...userInfo } = userInput;
    ...
    const newUser = await this.userRepository.save({
      ...userInfo, password: hashedPassword, userNumber: userNumber, temperature: temperatureId, areas: resultAreas
    });
    return newUser;
  }
  ...
  async setPwd({ userId, newPassword }) {
    const result = await this.userRepository.update({ userNumber: userId }, { password: newPassword });
    return result.affected ? true : false;
  }
  ...
  async findUser({ userNumber }) {
    const result = await this.userRepository.findOne({where: { userNumber },
    relations: ['temperature', 'areas']});
    return result;
  }
}
